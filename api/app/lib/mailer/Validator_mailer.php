	...
	class Validator_Mailer {
		...
		*/
		...
		public function validateRfc($key, $value, $rules) {
			...
			$cuartoValor = substr($value1, 3, 1);
			
        	//RFC Persona Moral.
        	if (ctype_digit($cuartoValor) && strlen($value1) == 12) {

        		$letras = substr($value1, 0, 3);
        		$numeros = substr($value1, 3, 6);
            	$homoclave = substr($value1, 9, 3);
            ...
            //RFC Persona Física.
       		} else if (ctype_alpha($cuartoValor) && strlen($value1) == 13) {

       			$letras = substr($value1, 0, 4);
            	$numeros = substr($value1, 4, 6);
            	$homoclave = substr($value1, 10, 3);
            	if (ctype_alpha($letras) && ctype_digit($numeros) && ctype_alnum($homoclave)) {
				...
				if( strlen($value1) <9 ||  strlen($value1) >13){
